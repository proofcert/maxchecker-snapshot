...
and exists_expert formula certificate storage =
  match Lkf_bureau.exists_expert certificate with
    ...
    | Some (certificate', term) ->
      let formula_filled = formula ((*Lkf_term.user*) term) in
...
and cut_expert certificate storage =
  match Lkf_bureau.cut_expert certificate with
    ...
    | Some (positive, negative, formula) ->
      let formula' = (*Lkf_formula.from_formula*) formula in
      let check_positive = check positive (Unfocused(storage, [formula']))
      and check_negative = check negative (Unfocused(storage, [neg formula'])) in
